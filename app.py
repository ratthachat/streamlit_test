...
st.title('LingoBot')
st.markdown('''
            *Powered by GPT-3 and Streamlit*
            
            All conversations are generated by GPT-3 which is intended for language practicing in everyday life in a non-toxic way.
...
def get_init_prompt(start_sentence, level, who_option):
  Jung_Lingo_init = "Write a conversation in the given \"Context\". The conversation will be polite, non-toxic and ordinary, no fantasy. Adjust vocabulary according to the given \"Level\". Open and end with #####\n\n"
